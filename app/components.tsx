...
export function CharacterCreator() {
  ...
  return (
    <div className='grid grid-col-1 md:grid-cols-12 w-full px-1'>
      ...
      <div className='flex flex-col justify-center text-center md:col-span-5  items-center mx-2'>
    ...
    <>      
      <header className="py-4 h-12">
        <div className='flex flex-row justify-start items-start text-start'>
          ...
          <input className={'p-1 w-full hover:bg-gray-500 '+ (selectedPage == 'Run' ? 'bg-white text-black' : '')} type={'button'} aria-label={'head_play'} value={'Run'} onClick={() => setSelectedPage('Run')}/>
        </div>
      </header>
...
function PlayPanel({type}:{type: string}){
  ...
  useEffect(() => {
    const handleSelectCharacterClick = (payload: { character: CharacterType }) => {
      ...
      setCharacters({...characters, [newName]: newChar })
    };
    ...
    <div className='flex flex-col justify-center '>
      ...
      }
      {
        ...
        <div className='grid grid-cols-1 md:grid-cols-12 '>
          ...
          <div className='flex flex-col justify-center md:col-span-5 gap-2 text-sm items-center'>
